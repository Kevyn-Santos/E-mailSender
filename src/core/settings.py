...
def cors_config(Urls: Any) -> list[str]:  # type: ignore
    ...
    if isinstance(Urls, str):
        ...
        return [
            u.strip() for u in clean.split(",") if u.strip()
        ]  # Se forem strings, retira os colchetes e espaços, depois separa por virgula
    else:
        ValueError(Urls)
...
class Settings(BaseSettings):
    ...
    def path_validator(self):
        ...
        if not filePath.is_file():  # type: ignore
            raise HTTPException(
                status_code=500, detail=f"Diretório não encontrado: {filePath}"
            )
        else:
            return filePath
